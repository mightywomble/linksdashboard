    ...
    for feed in feeds:
        try:
            ...
            if parsed_feed.entries:
                ...
                # Store pub_date for sorting but don't include in JSON response
                article['_sort_date'] = pub_date or datetime.now()
                latest_articles.append(article)
        except Exception as e:
            print(f"Error fetching latest from {feed['name']}: {str(e)}")
    
    # Sort by publication date (newest first)
    latest_articles.sort(key=lambda x: x['_sort_date'], reverse=True)
    
    # Remove the _sort_date before returning (not JSON serializable)
